...
async def finish_quiz(target: Message | CallbackQuery):
    ...
    # 1. Сначала отправляем расшифровку результата
    await msg_obj.answer(f"📌 Расшифровка → Результат\n\n{result_text}")

    # 2. ВКЛЮЧАЕМ ПАУЗУ (например, на 3 секунды)
    # Бот подождет, прежде чем отправить следующее сообщение
    await asyncio.sleep(3)

    # 3. Отправляем финальное сообщение с кнопками
    ...
    # Сброс сессии для возможности повторного прохождения
    SESSIONS[user_id] = UserSession()
