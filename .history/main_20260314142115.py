...
VARIANTS = {
    "v1": {
        ...
        "questions": [
            ...
            {"q": "8️⃣ Вы начинаете понимать неприятную правду.", "options": [("A", "It’s starting to sink in."), ("B", "I understand now."), ("C", "Now I clearly realize everything.")], "correct": "A"},
            ...
            {"q": "1️⃣1️⃣ Человек ведёт себя подозрительно.", "options": [("A", "Something feels off."), ("B", "He is not honest."), ("C", "His behavior is strange.")], "correct": "A"},
...
async def send_next_question(target: Message | CallbackQuery):
    ...
    # УБРАНО: теги <b> и </b> больше не добавятся к вопросу
    text = f"{q['q']}\n\nВыбери ответ:"
    
    markup = kb_question(q["options"])

    if isinstance(target, CallbackQuery):
        # Используем answer, чтобы вопрос приходил новым сообщением
