...
WEBINAR_URL = os.getenv("WEBINAR_URL", "https://default-url.com") # Можно указать запасную ссылку
CHANNEL_URL = os.getenv("CHANNEL_URL", "https://t.me/default_channel")
...
VARIANTS = {
    "v1": {
        ...
        "questions": [
            ...
            {"q": "2️⃣ Вы до сих пор не можете отпустить старую обиду.", "options": [("A", "I’m starting to resent it."), ("B", "I’m still upset about it."), ("C", "I still remember it.")], "correct": "A"},
            ...
            {"q": "5️⃣ Человеку нужно загладить вину.", "options": [("A", "He should apologize."), ("B", "He should make up for it."), ("C", "He should say sorry.")], "correct": "B"},
            {"q": "6️⃣ Разговор становится неловким.", "options": [("A", "This is awkward."), ("B", "It feels uncomfortable."), ("C", "The situation is not pleasant.")], "correct": "A"},
            {"q": "7️⃣ Вы эмоционально вымотаны отношениями.", "options": [("A", "It was stressful."), ("B", "It was very difficult for me emotionally."), ("C", "It drained me.")], "correct": "C"},
            {"q": "8️⃣ Вы начинаете понимать неприятную правду.", "options": [("A", "I understand now."), ("B", "It’s starting to sink in."), ("C", "Now I clearly realize everything.")], "correct": "B"},
            ...
            {"q": "🔟 Вы хотите честно высказать мнение.", "options": [("A", "Honestly…"), ("B", "To tell the truth in this situation…"), ("C", "If I’m being honest…")], "correct": "C"},
            ...
            {"q": "1️⃣4️⃣ Вы понимаете, что ситуация сложнее, чем казалось.", "options": [("A", "There’s more to it than I thought."), ("B", "It’s complicated."), ("C", "It is not simple as I believed.")], "correct": "A"},
...
def compute_result(variant_id: str, answers: List[str]) -> tuple[str, int]:
    ...
    if score <= 5:
        res = (
            ...
            "Вы говорите правильно. Грамматика на месте. Словарный запас есть.\n"
            ...
            "Это не проблема уровня. Это проблема типа лексики."
        )
    elif score <= 10:
        res = (
            "🟠 6–10 баллов\n"
            "“На границе перехода”\n\n"
            "Вы чувствуете живую речь. Иногда выбираете естественные формулировки.\n"
            ...
            "Вы уже не B1. Но ещё не звучите свободно.\n"
        ...
        res = (
            ...
            "Если вы не практикуете такие конструкции регулярно, они остаются пассивными.\n"
...
from aiogram.enums import ChatAction # Добавьте этот импорт в начало файла, если его нет
